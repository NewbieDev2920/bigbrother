...
import dataclasses
from src.domain.models import SecopInvias
from src.application.ports.crud_port import SecopInviasPort

class SQLiteSecopInviasAdapter(SecopInviasPort):
    ...
    def create_table(self) -> None:
        fields = dataclasses.fields(SecopInvias)
        ...
        create_table_q = f"""CREATE TABLE IF NOT EXISTS secopii_invias(
            {", ".join(columns)}
        );"""
        create_index_q = "CREATE INDEX IF NOT EXISTS idx_invias_nit ON secopii_invias(nit_entidad);"
    ...
    def _extract_tuple(self, record: SecopInvias) -> tuple:
    ...
    def update(self, key: str, record: SecopInvias) -> bool:
        ...
        update_q = f"UPDATE secopii_invias SET {set_clause} WHERE id_contrato=?"
    ...
    def delete(self, key: str) -> bool:
        delete_q = "DELETE FROM secopii_invias WHERE id_contrato=?"
        with self._get_connection() as conn:
            cursor = conn.cursor()
    ...
    def count_all(self) -> int:
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM secopii_invias")
            return cursor.fetchone()[0]

    def count_by_nit(self, nit_entidad: str) -> int:
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM secopii_invias WHERE nit_entidad=?", (nit_entidad,))
            return cursor.fetchone()[0]

    def get_by_nit(self, nit_entidad: str) -> List[SecopInvias]:
        query = "SELECT * FROM secopii_invias WHERE nit_entidad=?"
        results = []
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(query, (nit_entidad,))
            rows = cursor.fetchall()
            fields = [f.name for f in dataclasses.fields(SecopInvias)]
            for row in rows:
                data = dict(zip(fields, row))
                results.append(SecopInvias(**data))
        return results
