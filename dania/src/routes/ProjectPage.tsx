...
import { DaniaScoringSection } from '@/components/project/DaniaScoringSection';
    ...
    return (
      <PageShell contained={false}>
        ...
        <div className="mx-auto max-w-7xl space-y-6 px-4 py-6">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card className="flex flex-col items-center gap-4 p-6">
              <h2 className="text-xs font-semibold uppercase tracking-wider text-navy-300">
                Riesgo de corrupción (AI Context)
              </h2>
              <RiskGauge score={analysis?.riesgoCorrupcion ?? project.riesgoCorrupcion} />
              <div className="w-full">
                <div className="mb-1 flex items-center justify-between text-xs text-navy-300">
                  ...
                  <span className="font-mono text-navy-700">
                    {analysis?.trichotomousOutput === 'APTO' ? '100%' : 'En revisión'}
                  </span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-canvas-border">
                  <div
                    className="h-full rounded-full bg-navy-700 transition-all"
                    style={{ width: analysis?.trichotomousOutput === 'APTO' ? '100%' : '30%' }}
                  />
                </div>
              </div>
              <ProjectInfo project={project} analysis={analysis} />
            </Card>
          ...
          {/* Bottom: Dania Scoring */}
          <DaniaScoringSection tributaries={analysis?.tributaryList || []} />
        </div>
      </PageShell>
    );
  }
